...
const TMD = (function() {
    ...
    return {
        ...
        inject: function(article) {
            ...
            if (media) {
                let status_id = article.querySelector('a[href*="/status/"]').href.split('/status/').pop().split('/')
                    .shift();
        ...
        click: async function(btn, status_id, is_exist) {
            ...
            info['full-text'] = tweet.full_text.split('\n').join(' ').replace(/\s*https:\/\/t\.co\/\w+/g, '').replace(
                /[\\\/\|<>\*\?:"]/g,
                v => invalid_chars[v],
            );
            ...
            if (medias.length > 0) {
                ...
                medias.forEach((media, i) => {
                    info.url = media.type == 'photo'
                        ...
                        : media.video_info.variants.filter(n => n.content_type == 'video/mp4').sort((a, b) =>
                            b.bitrate - a.bitrate
                        )[0].url;
        ...
        settings: async function() {
            ...
            let dialog = $element(
                wapper,
                'div',
                'position: absolute; left: 50%; top: 50%; transform: translateX(-50%) translateY(-50%); width: fit-content; width: -moz-fit-content; background-color: #f3f3f3; border: 1px solid #ccc; border-radius: 10px;',
            );
...
`,
        ...
        fetchJson: async function(status_id) {
            let url = 'https://twitter.com/i/api/2/timeline/conversation/' + status_id
